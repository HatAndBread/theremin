...
<div class="absolute top-0 left-0 h-screen w-[80px] bg-base-200">
  <div class="flex justify-center mt-2">
    <Dropdown buttonLabel={"Instrument"} items={Object.keys(samples)} onClick={selectInstrument} buttonClass="btn btn-primary"><img src={iconSrc} slot="icon" alt="Instrument" class="w-[24px]"/></Dropdown>
  </div>
  ...
  <div class="w-full flex justify-center gap-2 mt-2">
    <button on:touchstart={record} class="bg-primary p-1 rounded" bind:this={recordBtn}>
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
  ...
  <a href="#my-modal-2" class="absolute bottom-0 m-1">
